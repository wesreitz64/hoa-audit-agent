...
- [x] Update `ingest_db.py` to save 100% of all parsed `homeowner_records` (52+ homeowners) into the SQLite Data Warehouse.
...
- **Vercel Serverless File System Boundary**: Deploying a Next.js dashboard that relies on a local SQLite database requires moving the `data/` directory inside the Next.js `dashboard/public/` folder. Setting Vercel's Root Directory to `dashboard` physically isolates it from the parent Git repository, meaning Next.js API Routes will fail with a 500 Server Error if they try to read `../data`.
